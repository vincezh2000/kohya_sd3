...
$pretrained_model = "./Stable-diffusion/sd3_medium.safetensors" # base model path | 底模路径
$vae = ""
...
$max_train_epoches = 16 # max train epoches | 最大训练 epoch
...
$lr_warmup_steps = 20 # warmup steps | 学习率预热步数，lr_scheduler 为 constant 或 adafactor 时该值需要设为0。仅在 lr_scheduler 为 constant_with_warmup 时需要填写这个值
$lr_decay_steps = 20 # decay steps | 学习率衰减步数，仅在 lr_scheduler 为warmup_stable_decay时 需要填写，一般是10%总步数
...
$output_name = "sd3" # output model name | 模型保存名称
...
elseif ($train_mode -ilike "hunyuan*") {
  $laungh_script = "hunyuan_train_network"
}
else {
...
if ($train_mode -ilike "*cn3l" -or $train_mode -ieq "controlnet") {
  ...
  elseif ($train_mode -ieq "sdxl_controlnet") {
    $laungh_script = "sdxl_train_controlnet"
  }
  else {
