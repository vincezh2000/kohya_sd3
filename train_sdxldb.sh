...
# Train Model | 训练模式
model="sdxl_db" #lora、db、sdxl_lora、sdxl_db、contralnet

# Train data path | 设置训练用模型、图片
pretrained_model="./Stable-diffusion/animagine-xl-3.0-base.safetensors" # base model path | 底模路径
is_v2_model=0                                                           # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
v_parameterization=0                                                    # parameterization | 参数化 v2 非512基础分辨率版本必须使用。
vae=""
train_data_dir="./train/higumei_hanyuu/train"          # train dataset path | 训练数据集路径
reg_data_dir="./train/higumei_hanyuu/reg"              # directory for regularization images | 正则化数据集路径，默认不使用正则化图像。
...
mixed_precision=""          # 默认fp16,可选 "fp16", "bf16","no"
...
optimizer_type="AdamW" # "adaFactor","AdamW8bit","Lion","DAdaptation",  推荐新优化器Lion。推荐学习率unetlr=lr=6e-5,tenclr=7e-6
...
fused_backward_pass=1 # use fused backward pass | 使用融合后的反向传播,训练大模型float32精度专用节约显存，必须优化器adafactor或者adamw，gradient_accumulation_steps必须为1或者不开。
